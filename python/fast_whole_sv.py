...
class sv_tools:
    ...
    def extract_reads(self, out_dir):
        ...
        os.system('samtools view -h {b} \
                  ...
                  .format(**mapping))
        os.system('samtools view -b  -f 4 -F264 {b}  > {o}{n}.tmps1.bam && \
                    samtools view -b -f 8 -F 260 {b}  > {o}{n}.tmps2.bam && \
...
@click.option('--split', '-s', help='path of split reads extract script', default='~/biosoftwares/lumpy-sv/scripts/extractSplitReads_BwaMem')
@click.option('--svaba', '-v', help='path of svaba programe', default='~/biosoftwares/svaba/bin/svaba')
