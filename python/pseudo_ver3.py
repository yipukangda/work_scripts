...
def cyp_p_des(sample, file_path, out_path, reference, marker_bed, extract_bed):
#    #trim and OC
...
#    file2 = file2.split('.')[0] + '_val_2.fq.gz'
    ...
    if not out_path.endswith('/'):
        out_path = out_path + r'/'
    mapping = {'n':sample, 'f':file_path, 'o':out_path, 'r':reference, 'm':marker_bed, 'e':extract_bed}
    os.system('ls {f}{n}* && /home/jgs/softerwares/bwa/bwa mem -M -t 16  -K 10000000 \
    ...
    os.system('samtools view {o}{n}_cyp21a2p_excluded.sorted.markdup.bam {e} | cut -f1 | uniq > {o}{n}_all.readid'.format(**mapping))
    with open('{o}{n}_all.readid'.format(**mapping),'r') as f:
        r1 = set(f.read().split('\n'))
    with open('{o}{n}_read_id.list'.format(**mapping),'r') as f:
        r2 = set(f.read().split('\n'))
    r = list(r1.difference(r2))
    with open('{o}{n}_exclude.id.list'.format(**mapping), 'w') as f:
        for i in r:
            f.write(i + '\n')
    os.system('java -jar /home/jgs/softerwares/picard/build/libs/picard.jar FilterSamReads \
              ...
              READ_LIST_FILE={o}{n}_exclude.id.list FILTER=excludeReadList'\
              .format(**mapping))
    os.system('java -jar /home/jgs/softerwares/picard/build/libs/picard.jar BuildBamIndex \
...
@click.option('--true', '-t', help='true gene cordinate,format like: 6:32006093-32009447',default='6:32006093-32009447')
def analysis_cyp_gene(filelist, inpath, outpath, refernece, bedfile, name, true):
    if filelist:
        f = open(filelist,'r')
        l = f.read().strip().split('\n')
        for i in l:
            cyp_p_des(i, inpath, outpath, refernece, bedfile, true)
        f.close()
    else:
        cyp_p_des(name, inpath, outpath, refernece, bedfile, true)
