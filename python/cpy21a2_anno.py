...
#    file2 = file2.split('.')[0] + '_val_2.fq.gz'
    ...
    os.system('bedtools multicov -bams {o}{n}_extract_bam.bam {o}{n}_cyp21a2p_excluded.sorted.markdup.bam \
              ...
              .format(**mapping))
    #Call variants with strelka2 germline mode
    with open('{o}{n}_strelka.sh'.format(**mapping), 'w') as sh:
      sh.write('/bioapps/strelka-2/bin/configureStrelkaGermlineWorkflow.py \
                    --bam {o}{n}_extract_bam.bam \
                    --exome \
                    --referenceFasta {r} \
                    --runDir {o} \n \
                    {o}runWorkflow.py -m local -j 8 \n \
                    chmod 777 -R {o}'\
                    .format(**mapping))
    os.system('docker run -it \
                      ...
                      -v /mnt:/mnt \
                      strelka2:2.8.4 \
                      bash {o}{n}_strelka.sh'.format(**mapping))
